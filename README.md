...
```bash
conda create -n demo_one python=3.9
conda activate demo_one
```
...
```
This command will create your personal repository under `/Repos/<username>/demo_one`.
...
- To trigger the release pipeline, get the current version from the `demo_one/__init__.py` file and tag the current code version:
```
